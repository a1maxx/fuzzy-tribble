...
import numpy as np
import pyomo.environ as pyo
from pyomo.environ import value, NonNegativeReals, RangeSet, TransformationFactory, Binary, Constraint, ConcreteModel
import random
from pyomo.gdp import Disjunction
from helper_functions import create_param_set
...
print(cluster_size, "/", value(model.o))

# %%
sum(value(model.x[i, 2, 1]) for i in range(1, 13))

[value(model.x[i, 3, 1]) for i in range(1, 13)]

for j, t in model.N:
    print(sum(value(model.x[i, j, t]) for i in model.M))

for i in zip(range(1, 13), range(14)):
    print(i)

for i in map(sum, (range(2), range(4))):
    print(i)
...
gen = infinite_sequence([1, 2], [2, 3])
next(gen)

a = {(i, int(j)): np.random.uniform(90, 120) for i, j in zip(range(1, 13), np.ones(12))}
