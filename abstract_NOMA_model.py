...
def solve_noma(cluster_size):
    ...
    model.G = pyo.Param(model.M, model.N,
                        initialize=dict(
                            zip((model.M * model.N), np.array(np.array([0.2, 0.1, 0.05, 0.01]).repeat(len(model.M)/4*(value(model.NOFS)))))))
    ...
    # return a.append([value(model.z[i, j]) for (i, j) in model.z.index_set()])
...
#
# transition_frequencies/sum(transition_frequencies)
# transition_frequencies/np.sum(transition_frequencies)
#
#
# n_10s = 52560
